...
// 실시간 진행 모니터링 — input/click 시 2초 debounce POST
function collectAnswers(){{
  const a={{}};
  document.querySelectorAll('.blank-input').forEach(el=>{{
    const k=el.dataset.id;
    if(k&&el.value) a[k]=el.value;
  }});
  document.querySelectorAll('.ox-group').forEach(grp=>{{
    const k=grp.dataset.id||grp.dataset.answer||'';
    const sel=grp.querySelector('.ox-btn.selected');
    if(sel) a['ox_'+k]=sel.textContent;
  }});
  return a;
}}
let progressTimer;
function sendProgress(){{
  ...
  const data={{
    type:'progress',
    worksheet:document.title,
    studentClass:cls,studentNumber:num,
    answers:collectAnswers(),
    ts:new Date().toISOString()
  }};
  fetch(SUBMIT_URL,{{method:'POST',mode:'no-cors',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(data)}}).catch(()=>{{}});
}}
function scheduleProgress(){{
  clearTimeout(progressTimer);
  progressTimer=setTimeout(sendProgress, 2000);
}}
document.querySelectorAll('.blank-input').forEach(el=>{{el.addEventListener('input', scheduleProgress);}});
document.querySelectorAll('.ox-btn').forEach(el=>{{el.addEventListener('click', scheduleProgress);}});
